...
public class Positions {
	...
	public Positions() {
	    ...
	    if (shapeColor == null) {
	    	p.showMessageDialog(null, "Cancel!", "Error", p.ERROR_MESSAGE);	  checkRead = false;   
	    } else if (shapeColor.equals("")) {
	    	p.showMessageDialog(null, "input is missing!", "Error", p.ERROR_MESSAGE);	checkRead = false;    
	    } else if (Integer.parseInt(shapeColor) < 1 || Integer.parseInt(shapeColor) > 6){
	    	p.showMessageDialog(null, "input is out of bounds!", "Error", p.ERROR_MESSAGE);	checkRead = false;    
	    }
	    else {
		    ...
		    if (shapeChanger == null) {
			    ...
			    if (height == null) {
				    ...
				    else if (Integer.parseInt(width) < 1 || Integer.parseInt(width) >= 200 ){
			    		p.showMessageDialog(null, "input is out of bounds!", "Error", p.ERROR_MESSAGE);	checkRead = false;    
			    	}
				    else {  
